...
import { VirtualProcessor as ConstrainedVP } from "./vm/underprovisionedVp/vp";
import {
  DEAD_CODE,
...
export class ControlFlowGraphBuilder {
  public constructor(
    ...
    private mode : "CONSTRAINED" | "NORMAL" = "NORMAL"
  ) {}
  ...
  public build(programs: Program[]): EngineProcessingResult {
    ...
    // const prgms = JSON.parse(JSON.stringify(programs));
    const enters = programs.map((program) => {
      ...
      const processor: VirtualProcessor | ConstrainedVP = this.mode === "NORMAL" ? new VirtualProcessor(
        listing,
        listener,
        optimizer,
        this.maxVMCount,
    ...
    // can't rely on the diagnostics generated by constrained run
    if (this.mode === "CONSTRAINED") diagnostics.clear();
