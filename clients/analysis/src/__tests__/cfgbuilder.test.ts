...
describe("contarined VM Control Flow Graph builder case tests", () => {
  const testsPath = path.join("src", "__tests__", "__resources__", "cfgraph");
  fs.readdirSync(testsPath)
    .filter((fname) => fname.endsWith("cfast.json"))
    .forEach((fname) => {
      it(`should work for ${fname}`, async () => {
        console.log(`\r\n\r\n\r\n******** constrained VM testing ${fname}`);

        const cfastJson = fs.readFileSync(path.join(testsPath, fname));
        const forest: Program[] = JSON.parse(cfastJson.toString());

        const primaryPath = path.join(testsPath, fname.replace(".cfast.json", ".constrained.result.dot"));
        const fallbackPath = path.join(testsPath, fname.replace(".cfast.json", ".result.dot"));

        const targetPath = fs.existsSync(primaryPath) ? primaryPath : fallbackPath;
        const expectedDot = fs.readFileSync(targetPath);
        const controlFlowBuilder = new ControlFlowGraphBuilder(
          40000,
          vscode.DiagnosticSeverity.Warning,
          undefined,
        ...
        // CAN"T RELY ON THE DIAGNOSTICS GENERATED BY CONSTRAINED VM
        // Check diagnostics
        // const diagnosticsFile = path.join(
        //   testsPath,
        //   fname.replace(".cfast.json", ".diagnostics.json"),
