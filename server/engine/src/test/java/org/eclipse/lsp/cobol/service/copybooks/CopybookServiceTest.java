...
class CopybookServiceTest {
  ...
  private final ResolveCopybookUri resolveCopybookUri = new CacheResolveCopybookUri(() -> client);
  ...
  void cacheInvalidationForNonImplicitCopybook() {
    ...
    verify(files, times(2)).getContentByPath(any(Path.class));
    verify(files, times(2)).getPathFromURI(DOCUMENT_3_URI);
  }
  ...
  void testSqldaCopybookResolutionDoesNotRelyOnBackend() {
    ...
    when(client.resolveCopybook(DOCUMENT_URI, SQLDA, "COBOL")).thenReturn(supplyAsync(() -> null));
