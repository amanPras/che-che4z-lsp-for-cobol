...
public abstract class CICSOptionsCheckBaseUtility {
  ...
  private final Map<Integer, String> baseDuplicateRulesOptions =
   ...
   */
  protected <E> void checkPrerequisiteIsMet(
      List<E> requiredContext, List<E> optionalContext, ParserRuleContext ctx, String options) {
    checkPrerequisiteIsMet(
        isNodePresent(requiredContext), isNodePresent(optionalContext), ctx, options);
  }

  protected <E> void checkPrerequisiteIsMet(
      E requiredContext, List<E> optionalContext, ParserRuleContext ctx, String options) {
    checkPrerequisiteIsMet(
        isNodePresent(requiredContext), isNodePresent(optionalContext), ctx, options);
  }

  protected <E> void checkPrerequisiteIsMet(
      List<E> requiredContext, E optionalContext, ParserRuleContext ctx, String options) {
    checkPrerequisiteIsMet(
        isNodePresent(requiredContext), isNodePresent(optionalContext), ctx, options);
  }

  protected <E> void checkPrerequisiteIsMet(
      E requiredContext, E optionalContext, ParserRuleContext ctx, String options) {
    checkPrerequisiteIsMet(
        isNodePresent(requiredContext), isNodePresent(optionalContext), ctx, options);
  }
  ...
  private void checkPrerequisiteIsMet(
      Boolean isRequiredContextPresent,
      Boolean isOptionalContextPresent,
  ...
  protected final <E> void checkMutuallyExclusiveOptions(String options, E... rules) {
    ...
    for (E rule : rules) {
      ...
      if (ParserRuleContext.class.isAssignableFrom(rule.getClass())
          || TerminalNode.class.isAssignableFrom(rule.getClass())) {
        rulesSeen++;
      ...
      if (rulesSeen > 1) {
        throwException(
            ErrorSeverity.ERROR,
            getLocality(isRuleList ? ((List<?>) rule).get(0) : rule),
   ...
   * @param options Options checked to insert into error message
   */
  protected void checkHasObsoleteOptions(
      List<TerminalNode> rule, ParserRuleContext ctx, String options) {
    if (!rule.isEmpty()) {
   ...
   */
  @SafeVarargs
  protected final <E> void checkHasAtLeastOneOption(
      String options, ParserRuleContext ctx, E... rules) {
    int rulesSeen = 0;
  ...
  private void processDuplicateRules(ParserRuleContext ctx, Map<Integer, String> subruleOptions) {
    ...
    for (ParserRuleContext child : ctx.getRuleContexts(ParserRuleContext.class)) {
      ...
      throwException(
          ErrorSeverity.ERROR,
          getLocality(child),
          "Options \"" + name + "\" cannot be used more than once in a given command.",
          "");
    }
  }
  ...
  protected void checkDuplicates(
      ParserRuleContext ctx,
      Map<Integer, ErrorSeverity> customDuplicateOptions,
      Map<Integer, String> customDuplicateRuleOptions) {
   ...
   */
  @SafeVarargs
  protected final int checkHasMutuallyExclusiveOptions(
   ...
   */
  @SafeVarargs
  protected final <E extends ParseTree> void checkAllOptionsArePresentOrAbsent(
      String options, ParserRuleContext ctx, List<?>... rules) {
      ...
      if (TerminalNode.class.isAssignableFrom(rule.getClass())) {
        if (ArrayUtils.contains(coreTokenIndex, ((TerminalNode) rule).getSymbol().getType())) {
          if (traversalIndex + 1 < ctx.children.size()) {
            ...
            if (child == null
                || (TerminalNode.class.isAssignableFrom(child.getClass())
                    && ((TerminalNode) child).getSymbol().getType() != CICSLexer.LPARENCHAR)) {
              throwBrowsingViolation(
  ...
  public <E extends ParserRuleContext> void checkBrowsingCommon(E ctx, int coreToken) {
    ...
    CICSParser.Cics_browse_start_endContext startEndCtx =
        ctx.getChild(CICSParser.Cics_browse_start_endContext.class, 0);
    if (startEndCtx != null) {
